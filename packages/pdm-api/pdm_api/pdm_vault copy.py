...
try:
    ...
    raise ImportError(f"PDM Interop DLL '{utils.PDM_DLL_NAME}' could not be loaded. Ensure it's accessible and dependencies are met. Error: {e}") from e
except Exception as e: # More general exception for clr.AddReference or other loading issues
    raise RuntimeError(f"An unexpected error occurred while loading the PDM DLL or its types: {e}") from e
...
class PDMVault:
    ...
    @profile
    def __init__(self, username: str, password: str, vault_name: str):
    ...
    def search_files(self,
        ...
        if not filename_pattern and not variable_conditions and state_id_filter is None:
            filename_pattern = '*'
        ...
        try:
            ...
            if variable_conditions:
                try:
                    if not hasattr(search_object, 'AddVariable2'):
                        raise PDMOperationFailedError("The obtained search object (potentially older interface) is missing the 'AddVariable2' method. Cannot apply variable conditions.")
                    for var_name, condition in variable_conditions.items():
                        search_object.AddVariable2(var_name, condition)
                except AttributeError as ae:
                    raise PDMOperationFailedError(f"AttributeError while trying to add variable conditions (check search object interface): {ae}") from ae
                except Exception as var_err:
                    raise PDMOperationFailedError(f"Error calling AddVariable2 for Var='{var_name}', Cond='{condition}': {var_err}") from var_err

            if state_id_filter is not None:
                try:
                    if hasattr(search_object, 'State'):
                        search_object.State = state_id_filter

                except Exception as state_err:
                    raise PDMOperationFailedError(f"Error setting State property with ID {state_id_filter}: {state_err}") from state_err


            result = search_object.GetFirstResult()
            
            while result:
                yield utils.format_search_result(result)
                try:
            ...
            raise PDMOperationFailedError(f"Unexpected error encountered during PDM file search: {e}") from e
    ...
    def _get_version_number_from_revision_label(self, file_obj: IEdmFile17, revision_label: str) -> int:
        ...
        file_name_for_log = file_obj.Name if hasattr(file_obj, 'Name') else "UnknownFile"
        file_id_for_log = file_obj.ID if hasattr(file_obj, 'ID') else "N/A"
        target_revision_upper = revision_label.strip().upper()

        try:
            ver_enum: Optional[Any] = None
            try:
            ...
            all_versions_for_revision: List[int] = []
                ...
                try:
                    ...
                    if rev_obj_raw:
                        ...
                        if rev_obj and hasattr(rev_obj, 'Name') and hasattr(rev_obj, 'VersionNo'):
                            ...
                            current_version_no = rev_obj.VersionNo

                            if current_rev_name == target_revision_upper:
                                all_versions_for_revision.append(current_version_no) # FIXED: uses .append()
            ...
            if all_versions_for_revision:
                latest_ver_for_rev = all_versions_for_revision[-1] # Gets the highest version number for that revision
                return latest_ver_for_rev
            else:
                return 0
